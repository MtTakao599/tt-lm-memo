...
function MemoPrintItem({
  ...
  return (
    <section className={variant === 'single' ? 'print-memo is-single' : 'print-memo'}>
      {title ? <h2>{title}</h2> : null}
      <dl className="print-memo-fields">
        <div>
          <dt>棟</dt>
          <dd>{memo.building}</dd>
        </div>
      ...
      {memo.photos.length > 0 ? (
        <div className={variant === 'single' ? 'print-photos is-single' : 'print-photos'}>
          <p>写真（{memo.photos.length}枚）</p>
          <div className="print-photo-list">
            {memo.photos.map((photo) => (
              <figure key={photo.id} className="print-photo-item">
        ...
        {memos.length === 0 ? (
          ...
          memos.map((memo, index) => (
            <MemoPrintItem
              ...
              memo={memo}
              variant={mode}
              title={isSingle ? undefined : `メモ ${index + 1}`}
            />
          ))
        )}
      </article>
